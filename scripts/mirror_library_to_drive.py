...
def _maybe_rewrite_internal_md_links_to_drive_pdf_links(
    *,
    body_html: str,
    md_path: Path,
    data_root: Path,
) -> str:
    """Optionally rewrite links to other library .md files into Drive PDF links.

    This is a best-effort enhancement used only when a Drive link manifest exists.
    The manifest is generated by the PDF mirror sync step.
    """

    manifest_path = CACHE_DIR / "drive_pdf_links.json"
    ...
    if not isinstance(manifest, dict) or "md_to_drive_pdf" not in manifest:
        return body_html
    md_to_drive_pdf = manifest.get("md_to_drive_pdf")
    if not isinstance(md_to_drive_pdf, dict):
        return body_html

    # Replace <a href="...md"> where the target resolves to a library file.
    ...
    def repl(m: re.Match[str]) -> str:
        ...
        try:
            abs_target = (md_path.parent / href_path).resolve()
            abs_target.relative_to(data_root.resolve())
        except Exception:
            return m.group(0)

        rel_md = abs_target.relative_to(data_root).as_posix()
        drive_url = md_to_drive_pdf.get(rel_md)
        if not isinstance(drive_url, str) or not drive_url.startswith(("http://", "https://")):
            return m.group(0)

        return f"{m.group(1)}{drive_url}{m.group(3)}"
...
def render_markdown_to_html(md_path: Path, css_text: str, template_text: str) -> str:
    ...
    # Nice-to-have: if we have a Drive PDF manifest, make internal library links
    # point to the corresponding PDFs on Drive.
    body_html = _maybe_rewrite_internal_md_links_to_drive_pdf_links(
        body_html=body_html,
        md_path=md_path,
        data_root=DATA_ROOT,
    )
...
def _drive_upload_pdf(
    ...
    *,
    ...
    if existing:
        ...
        return service.files().get(fileId=existing["id"], fields="id,webViewLink").execute()
    else:
        ...
        if isinstance(created, dict) and created.get("webViewLink"):
            return created
        if isinstance(created, dict) and created.get("id"):
            return service.files().get(fileId=created["id"], fields="id,webViewLink").execute()
        return None
...
def sync_pdfs_to_drive(
    *,
    ...
    # Manifest for the nice-to-have feature: internal `.md` links in PDFs can point
    # to the corresponding PDFs on Drive.
    md_to_drive_pdf: dict[str, str] = {}
    ...
    for pdf_path in pdf_files:
        ...
        uploaded_meta = _drive_upload_pdf(
            service,
            parent_id=parent_id,
            local_pdf=pdf_path,
        ...
        # Record Drive link for this PDF (best effort; only when not dry-run).
        if not dry_run and isinstance(uploaded_meta, dict):
            web_view = uploaded_meta.get("webViewLink")
            if isinstance(web_view, str) and web_view.startswith(("http://", "https://")):
                md_rel = pdf_path.relative_to(local_pdf_root).with_suffix(".md").as_posix()
                md_to_drive_pdf[md_rel] = web_view
        uploaded += 1

    if not dry_run and md_to_drive_pdf:
        _safe_mkdir(CACHE_DIR)
        (CACHE_DIR / "drive_pdf_links.json").write_text(
            json.dumps({"md_to_drive_pdf": md_to_drive_pdf}, indent=2, sort_keys=True),
            encoding="utf-8",
        )
