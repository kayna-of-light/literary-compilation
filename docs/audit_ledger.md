...
| **Swedenborg cited by Drive-hosted PDF filename** | A bibliography entry reading `swedenborg\_apocalypse\_revealed\_02.pdf` plus a `drive.google.com/open?id=…` link — no work title, no section, uncheckable. | Name the work and date in the document's own Works Cited form, and **keep** the Drive link labelled as the author's scan (§ 3.4: preserve the chain). Log any unresolved section reference in the research-question register — **do not** write a `[P]`/`[S]`/`[T]` type code or a `[TRACE NEEDED]` tag into the entry itself; grep across `data/` turned up no document that tags individual Works Cited entries that way, so the first pass on 2026-09-21 introduced a discrepancy rather than fixing one, corrected the same day once flagged (see § 3.4a of the procedure). `scripts/normalize_internal_links.py` cannot help — it needs absent Drive credentials and only matches links resolving to Markdown in `data/`. All 3 instances corpus-wide were cleared on 2026-09-21; the pattern is recorded in case new Deep Research output reintroduces it. | `00_Framework/The Heart of the Matter…`, `02_Swedenborgian_Theology/The Biological Error…`, `05_The_Self/The Architecture of Autonomy…` (2026-09-21) |
