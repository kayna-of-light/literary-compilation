...
>
> **The discipline is holding.** Tonight: two documents read in full, **four token edits and two Works Cited
> reformats, zero lines of prose**, and nine findings deliberately left for the author (four research
> questions). Read § 3.6 before touching a file; the full `git diff origin/dev -- data/` should still fit
> on one screen and contain nothing but changed tokens and reformatted entries.
>
...
>
> **Awaiting external answers**: 13 research questions from the audit are open — 9 from earlier nights (see the
> previous blocks in `docs/research_questions.md`) and **4 logged tonight** in the "2026-09-23" block. The one
> most worth an author's minute: the *Apocalypse Revealed* §11 attribution — both Blavatsky theses credit the
> "seek for it in China" directive to *Doctrine of the Sacred Scripture*, which does not contain it (§§101–103 read
> in full). The proposed fix is written out in the question. Second: *The Protective Garment* says Blavatsky
> "read the *Apocalypse of Adam*" — a Nag Hammadi text found 54 years after her death; *The Empty Room*, its
> later rewrite, already dropped the claim. Whether to annotate, revise or retire the earlier sibling is the
> author's call.
>
> **Worth knowing**:
