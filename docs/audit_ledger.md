...
>
> **The most important thing tonight found — read before touching `06_Mythological_Studies` or `00_Master_Theses`**:
> four phrases *The River, Not the Chain* quotes as Lovejoy's own ("does not acknowledge that extracting an idea
> from its system strips or distorts its essential nature", "artificial freezing", "inherently demanded",
> "temporarily paralyzed") are in **neither** *The Great Chain of Being* nor the 1911 *Monist* essay — checked
> against full texts. They carry the thesis's central "Lovejoy confesses" move, and *The Downstream Refinery*
> (`00_Master_Theses`) repeats two of them. Logged `[GDR]` HIGH; not edited, because the fix changes the argument.
> The sibling *Descent Remembered as Ascent* was **not** checked for the same phrases (grep found none, but it may
> paraphrase them) — it is the natural next pick.
>
